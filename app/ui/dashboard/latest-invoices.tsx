...
export default function LatestInvoices({ latestInvoices }: { latestInvoices: LatestInvoice[] }) {
  return (
    <div className="flex w-full flex-col md:col-span-4">
      <h2 className={`${lusitana.className} mb-4 text-xl md:text-2xl`}>Latest Clients</h2>
            ...
            >
              <div className="flex items-center">
                ...
                <div className="ml-4 min-w-0">
                  <p className="truncate text-sm font-semibold md:text-base">{client.name}</p>
                  {client.email && (
                    <p className="hidden text-sm text-gray-500 sm:block">{client.email}</p>
                  )}
                </div>
              </div>
            </div>
